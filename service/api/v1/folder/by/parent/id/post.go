...
var Post = session.Authenticate(
	func(w http.ResponseWriter, r *http.Request, p httprouter.Params, s session.Session) {
		...
		if config.BuildDebug == true {
			fmt.Println(`==> POST: /service/api/v1/folder/by/parent/id/:fid`)
		}
		...
		err = folder.AddChildFolderForFolderIDFromUserID(requestData.Name, folderID, userID)
		if err == folder.ErrShortName {
			log.RespondJSON(w, `{}`, http.StatusPreconditionFailed)
			return
		} else if err == folder.ErrUserNotAllowed {
			log.RespondJSON(w, `{}`, http.StatusForbidden)
			return
		} else if err != nil {
